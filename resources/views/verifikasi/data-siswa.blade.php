                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <p class="text-gray-800">
                            @php
                            $pembayaran = $formulir->pembayaran;
                            $statusBayar = $pembayaran->status ?? null;
                            @endphp
                            @if($statusBayar)
                            <span class="inline-block px-3 py-1 rounded-full text-sm font-semibold
                                    @if($statusBayar == 'Lunas' || $statusBayar == 'completed' || $statusBayar == 'settlement')
                                        bg-green-100 text-green-800
                                    @elseif($statusBayar == 'Pending' || $statusBayar == 'pending')
                    ...
                    <div>
                        ...
                        <p class="text-gray-800">
                            @php
                            $statusVerif = $formulir->status_verifikasi ?? 'menunggu';
                            @endphp
                            <span class="inline-block px-3 py-1 rounded-full text-sm font-semibold
                                @if($statusVerif == 'diverifikasi' || $statusVerif == 'verified')
                                    bg-green-100 text-green-800
                                @elseif($statusVerif == 'menunggu' || $statusVerif == 'pending')
                                    bg-yellow-100 text-yellow-800
                                @else
                                    bg-red-100 text-red-800
                                @endif
